...
import codeping.flex.stock.adapter.out.persistence.mapper.StockFundamentalMapper;
import codeping.flex.stock.adapter.out.persistence.mapper.StockIDMapper;
import codeping.flex.stock.application.mapper.StockFundamentalInfoDtoMapper;
...
import codeping.flex.stock.domain.stockData.*;
...
public class StockInfoService implements StockInfoUsecase {
    ...
    private final LoadStockFundamentalPort loadStockFundamentalPort;

    private final StockInfoResponseMapper stockInfoResponseMapper;
    private final StockFundamentalInfoDtoMapper stockFundamentalInfoDtoMapper;
    ...
    @Override
    public StockFundamentalInfoDto getStockFundamentalInfo(String stockcode, LocalDate date) {
         StockFundamental stockFundamental = loadStockFundamentalPort.loadStockFundamental(stockcode, date.toString())
                 .orElseThrow(() -> ApplicationException.from(StockErrorCode.STOCK_NOT_FOUND));
         return stockFundamentalInfoDtoMapper.toDto(stockFundamental);
    }
